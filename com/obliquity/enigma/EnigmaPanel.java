...
public class EnigmaPanel extends Component {
    static final int INACTIVE = 0, CAPTION = 1, FILLING = 2, EVALUATED = 3, MASTER = 4;
	...
	nWidth = nNumBeads * (nBeadSize + nPadding) + nPadding +
	    nBeadsWide * (nPegSize + nPadding);
    ...
    public int getNumBeads() { return nNumBeads; }

    public int getBeadCount() { return nBeads; }

    public boolean isFull() { return (nBeads == nNumBeads); }
    public boolean isEmpty() { return (nBeads == 0); }
	...
	if (i < 0 || i >= nNumBeads)
	    throw new Exception("Invalid index " + i + " to EnigmaPanel of size " + nNumBeads);
	...
	if (nBeads < nNumBeads)
	    throw new Exception("Cannot evaluate incomplete EnigmaPanel: only " + nBeads +
				" set but " + nNumBeads + " required");

	if (nNumBeads != other.getNumBeads())
	    throw new Exception("Cannot evaluate this EnigmaPanel (" + nNumBeads +
				") against one with " + other.getNumBeads());

	if (!other.isFull())
	    throw new Exception("Cannot evaluate against incomplete EnigmaPanel");
